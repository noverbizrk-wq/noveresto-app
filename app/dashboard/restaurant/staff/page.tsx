    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <h1 style={{ fontSize: 24, fontWeight: 800, fontFamily: 'serif', color: '#fff' }}>
          👥 Équipe <span style={{ color: '#00C48C' }}>et planning</span>
        </h1>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <h2 style={{ color: '#fff', fontSize: 16, fontWeight: 700 }}>Employés</h2>
          <button
        ...
        {showEmployeeForm && (
          <div style={{ background: '#0F2D40', border: '1px solid #1A3A52', borderRadius: 12, padding: 16, marginBottom: 16, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <input placeholder="Nom" value={newEmployee.name} onChange={(e) => setNewEmployee({ ...newEmployee, name: e.target.value })}
              style={{ background: '#081522', border: '1px solid #1A3A52', borderRadius: 8, padding: '6px 10px', color: '#fff', fontSize: 13, flex: 1, minWidth: 160 }} />
            ...
            <button onClick={createEmployee} style={{ padding: '6px 16px', borderRadius: 8, border: 'none', background: '#00C48C', color: '#081522', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
              Créer
            </button>
          </div>
        )}
        ...
        <div style={{ background: '#0F2D40', border: '1px solid #1A3A52', borderRadius: 12, overflow: 'hidden' }}>
          <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {employees.map((e) => (
                <tr key={e.id} style={{ borderTop: '1px solid #1A3A52' }}>
                  <td style={{ padding: 12, color: '#fff', fontWeight: 600 }}>{e.name}</td>
                  <td style={{ padding: 12, color: '#8BAABF' }}>{ROLE_LABELS[e.role] || e.role}</td>
                  <td style={{ padding: 12, color: '#8BAABF' }}>{Number(e.hourly_cost).toFixed(3)} TND/h</td>
                  <td style={{ padding: 12 }}>
                    <span style={{ padding: '3px 10px', borderRadius: 999, fontSize: 11, fontWeight: 600, background: e.is_active ? 'rgba(0,196,140,.15)' : 'rgba(106,143,171,.15)', color: e.is_active ? '#00C48C' : '#6A8FAB' }}>
          ...
          {!loading && employees.length === 0 && (
            <p style={{ color: '#6A8FAB', padding: 16 }}>Aucun employé. Créez-en un avec le bouton ci-dessus.</p>
          )}
        </div>
      </div>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <h2 style={{ color: '#fff', fontSize: 16, fontWeight: 700 }}>Planning (créneaux à venir)</h2>
          <button
        ...
        {showShiftForm && (
          <div style={{ background: '#0F2D40', border: '1px solid #1A3A52', borderRadius: 12, padding: 16, marginBottom: 16, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            <input type="datetime-local" value={newShift.ends_at} onChange={(e) => setNewShift({ ...newShift, ends_at: e.target.value })}
              style={{ background: '#081522', border: '1px solid #1A3A52', borderRadius: 8, padding: '6px 10px', color: '#fff', fontSize: 13 }} />
            <button onClick={createShift} style={{ padding: '6px 16px', borderRadius: 8, border: 'none', background: '#00C48C', color: '#081522', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
              Planifier
            </button>
          </div>
        )}
        ...
        <div style={{ background: '#0F2D40', border: '1px solid #1A3A52', borderRadius: 12, overflow: 'hidden' }}>
          <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {shifts.map((s) => (
                <tr key={s.id} style={{ borderTop: '1px solid #1A3A52' }}>
                  <td style={{ padding: 12, color: '#fff', fontWeight: 600 }}>{s.employee_name}</td>
                  <td style={{ padding: 12, color: '#8BAABF' }}>{new Date(s.starts_at).toLocaleString('fr-FR')}</td>
                  <td style={{ padding: 12, color: '#8BAABF' }}>{new Date(s.ends_at).toLocaleString('fr-FR')}</td>
                  <td style={{ padding: 12, color: '#8BAABF' }}>{s.status}</td>
                  <td style={{ padding: 12 }}>
                    <button onClick={() => deleteShift(s.id)} style={{ background: 'transparent', border: 'none', color: '#E84545', cursor: 'pointer', fontSize: 12 }}>Retirer</button>
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          {!loading && shifts.length === 0 && (
            <p style={{ color: '#6A8FAB', padding: 16 }}>Aucun créneau planifié.</p>
          )}
        </div>
      </div>
    </div>
